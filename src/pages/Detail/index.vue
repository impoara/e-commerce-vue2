			...
			<div class="mainCon">
				...
				<div class="InfoWrap">
					...
					<div class="choose">
						...
						<div class="cartWrap">
							<div class="controls">
								<input autocomplete="off" class="itxt" v-model="skuNum" @focusout="skuNumSetText">
								<a href="javascript:" class="plus" @click="skuNumSetBtn('+')">+</a>
								<a href="javascript:" class="mins" @click="skuNumSetBtn('-')">-</a>
							</div>
							<div class="add">
...
<script>
	...
	export default {
		...
		methods: {
			...
			skuNumSetBtn(set) {
				if (set == '+') {
					if (this.skuNum < 9999) this.skuNum++;
				} else {
			...
			async addShopCart() {
				...
				this.spuSaleAttrList.forEach(item => {
					item.spuSaleAttrValueList.forEach(item => {
						if(item.isChecked == '1'){
							let key = item.saleAttrName
							let value = item.saleAttrValueName
							let props = `${key}:${value}`
							this.skuGoodInfo.push(props)
						}
					})
				})
				//发请求----将产品加入到购物车中(通知服务器,并接收返回信息[是否成功])
				try {
					let result = await this.$store.dispatch('addOrdShopCart', {
						skuId: this.$route.params.skuid,
						skuNum: this.skuNum
					})
					//进行路由跳转,在路由跳转的时候将产品信息带给到下一级路由组件
					//1.这种传参方式会导致地址栏乱码(但是也能用)
					// this.$router.push({name:'addcartsuccess',query:{skuInfo:this.skuInfo,skuNum:this.skuNum}})
					//2.产品信息的数据[比较复杂:skuInfo],通过会话存储(不持久,会话结束数据再消失)
					sessionStorage.setItem("SKUINFO",JSON.stringify(this.skuInfo))
					this.$router.push({name:'addcartsuccess',query:{skuNum:this.skuNum,skuGoodInfo:this.skuGoodInfo}})
				} catch (error) {
					alert(error.message)
				}
			}
