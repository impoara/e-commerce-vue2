	...
	<div class="trade-container">
		...
		<div class="content">
			...
			<div class="detail">
				...
				<ul class="list clearFix" v-for="(order) in orderInfo.detailArrayList" :key="order.skuId">
					<li>
						<img v-bind:src="order.imgUrl" alt="" style="height: 95px;">
					</li>
					<li>
						<p>{{order.skuName}}</p>
						<h4>7天无理由退货</h4>
		...
		<div class="trade">
			...
			<div class="receiveInfo">
				寄送至: <span style="color: #000000;">{{userDefaultAddress.fullAddress}}</span>&nbsp;&nbsp;&nbsp;&nbsp;
				收货人: <span style="color: #000000;">{{userDefaultAddress.consignee}}</span>&nbsp;&nbsp;&nbsp;
				手机号: <span style="color: #000000;">{{userDefaultAddress.phoneNum}}</span>
			</div>
		</div>
...
<script>
	...
	export default {
		...
		methods: {
			//切换收件人地址信息
			async changeDefault(address,addressInfo) {
				await addressInfo.forEach((item)=>{
					item.isDefault = '0'
				})
				address.isDefault = '1'
			},
			...
			async submitOrder(){
				...
				let data = {
					...
					orderDetailList: this.orderInfo.detailArrayList,//商品列表清单
				}
				//需要带参:tradeNo、data
